...
{
    int total = 0;
    for (auto roman : romanInput)
    {
        std::string minimal = convert(convert(roman));
        std::cout << roman << " " << convert(roman) << " " << minimal << " " << roman.size() - minimal.size() << std::endl;

        total += roman.size() - minimal.size();
    }
